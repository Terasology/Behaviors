...
public class TerritorialBehaviourSystem extends BaseComponentSystem implements UpdateSubscriberSystem {
    private static final Logger logger = LoggerFactory.getLogger(TerritorialBehaviourSystem.class);
    private final List<Vector3f> territories = new ArrayList<Vector3f>();
    ...
    public void update(float delta) {
        for (EntityRef entity : entityManager.getEntitiesWith(TerritoryDistance.class, LocationComponent.class)) {
            ...
            territoryDistance.distanceSquared =
                    territoryDistance.location.distanceSquared(entity.getComponent(LocationComponent.class).getWorldPosition());
